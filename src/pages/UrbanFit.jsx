...
import { HiOutlineCurrencyDollar, HiOutlineClipboardList, HiOutlineTruck, HiOutlinePlus } from 'react-icons/hi';
...
const UrbanFitPanel = () => {
  ...
  const loadData = async () => {
    try {
      const [overviewRes, ordersRes, salesRes] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 bg-dark-800 p-1 rounded-lg overflow-x-auto">
        {['overview', 'orders', 'daily-sales'].map(tab => (
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(null)}>
          <div className="bg-dark-800 rounded-xl border border-dark-700 w-full max-w-lg max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
            <h3 className="text-xl font-bold text-white mb-6">
              {showModal === 'order' ? 'New Stitching Order' : 'Record Daily Sales'}
            </h3>
            <form onSubmit={showModal === 'order' ? handleCreateOrder : handleRecordSales} className="space-y-4">
              {showModal === 'order' ? (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div><label className="label">Advance Paid</label>
                      <input type="number" className="input" value={form.advance_paid||''} onChange={e=>setForm({...form,advance_paid:e.target.value})} /></div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                ...
                <>
                  ...
                  <div><label className="label">Total Sales</label>
                    <input type="number" className="input" required value={form.total_sales||''} onChange={e=>setForm({...form,total_sales:e.target.value})} /></div>
                  <div className="grid grid-cols-3 gap-4">
                    <div><label className="label">Cash</label>
                      <input type="number" className="input" value={form.cash_sales||''} onChange={e=>setForm({...form,cash_sales:e.target.value})} /></div>
                    <div><label className="label">Card</label>
                      <input type="number" className="input" value={form.card_sales||''} onChange={e=>setForm({...form,card_sales:e.target.value})} /></div>
                    <div><label className="label">Online</label>
                      <input type="number" className="input" value={form.online_sales||''} onChange={e=>setForm({...form,online_sales:e.target.value})} /></div>
                  </div>
                  <div><label className="label">Items Sold</label>
                    <input type="number" className="input" value={form.items_sold||''} onChange={e=>setForm({...form,items_sold:e.target.value})} /></div>
                </>
              )}
